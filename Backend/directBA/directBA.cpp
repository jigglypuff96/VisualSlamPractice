...
public:
    ...
    virtual void computeError() override {
        ...
        Eigen::Vector3d XYZ = v1->estimate() * v2->estimate();
...
int main(int argc, char **argv) {
    ...
    for (auto p:poses) {
        ...
        vertex_pose->setId(index + points.size());
        if (index == 0 ) {vertex_pose->setFixed(true);}
        vertex_pose->setEstimate(poses[index++]);
    ...
    for ( auto p:points )
    {
        ...
        point->setId ( index );
        point->setEstimate ( points[index++]);
    ...
    for (int i = 0; i < poses.size(); i++) {
        VertexSophus *pose_vertex_new = dynamic_cast<VertexSophus* >(optimizer.vertex(i + points.size()));
