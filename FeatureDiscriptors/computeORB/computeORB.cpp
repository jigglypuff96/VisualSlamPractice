...
int main(int argc, char **argv) {
    ...
    if ( first_image.data == nullptr ) //数据不存在,可能是文件不存在
    {
        cerr<<"文件不存在1."<<endl;
        return 0;
    }
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	if (u >= half_patch_size && u+half_patch_size < image.cols && v >= half_patch_size && v+half_patch_size< image.rows){
	    double m01 = 0, m10 = 0;
        for(int xB = -half_patch_size; xB < half_patch_size; xB++)
        {
            for(int yB = -half_patch_size; yB < half_patch_size; yB++)
            {
                auto I = image.at<uchar>(v+yB,u+xB);
                m01 += yB*I;
                m10 += xB*I;
            }
        }
        kp.angle=(float)(atan2(m01,m10)/pi*180);
    }
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        auto theta = kp.angle*pi/180;
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            double up = ORB_pattern[4*i], vp = ORB_pattern[4*i+1], uq = ORB_pattern[4*i+2], vq = ORB_pattern[4*i+3];
            double up_prime = cos(theta)*up - sin(theta)*vp + kp.pt.x;
            double vp_prime = sin(theta)*up + cos(theta)*vp + kp.pt.y;
            double uq_prime = cos(theta)*uq - sin(theta)*vq + kp.pt.x;
            double vq_prime = sin(theta)*uq + cos(theta)*vq + kp.pt.y;

            if(up_prime < 0 || up_prime > image.cols || vp_prime < 0 || vp_prime > image.rows
               || uq_prime < 0 || uq_prime > image.cols || vq_prime < 0 || vq_prime > image.rows){
                d.clear();
                break;
            }else{
                double Ip_prime = image.at<uchar>(vp_prime,up_prime);
                double Iq_prime = image.at<uchar>(vq_prime,uq_prime);
                if (Ip_prime > Iq_prime){
                    d[i] = 0;
                }else{
                    d[i] = 1;
                }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    cout<<"Time = "<<((int)clock()-time0)/(double)CLOCKS_PER_SEC<<"s"<<endl;
    // END YOUR CODE HERE
    for (auto &m: matches) {
