        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    auto u1 = double(kp.pt.x + x);
                    auto v1 = double(kp.pt.y + y);
                    double u2 = u1 + dx;
                    double v2 = v1 + dy;
                    double error = 0;
                    ...
                    if (!inverse) {
                        // Forward Jacobian
                        J[0] = (GetPixelValue(img2, u2 + 1, v2) - GetPixelValue(img2, u2 - 1, v2))/2;
                        J[1] = (GetPixelValue(img2, u2, v2 + 1 ) - GetPixelValue(img2, u2, v2 - 1))/2;
                    } else {
                        // Inverse Jacobian
                        // NOTE this J does not change when dx, dy is updated, so we can store it and only compute error
                        J[0] = (GetPixelValue(img1, u1 + 1, v1) - GetPixelValue(img1, u1 - 1, v1))/2;
                        J[1] = (GetPixelValue(img1, u1, v1 + 1) - GetPixelValue(img1, u1, v1 - 1))/2;
                    }
...
void OpticalFlowMultiLevel(
    ...
    for (int k = pyramids-1; k >= 0; k--)
    {
        ...
        {
            KeyPoint kp1_scale = kp1[j];
            kp1_scale.pt *= scales[k];
            kp1_now.push_back(kp1_scale);
        }
        OpticalFlowSingleLevel(pyr1[k], pyr2[k], kp1_now, kp2, success, inverse);
        kp2_last = kp2;
        for (int j = 0; j < kp1.size(); j++)
        {
